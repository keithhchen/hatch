...
已通过的自动验证（2026-08-11 当前 worktree）：Renderer `22 files / 96 tests`；Rust `44 passed / 1 ignored Keychain smoke`；Runtime Node test `226 passed`；LocalRunner `43 passed`；Web build；Tauri preview/release `.app` build；production-session `cargo check`。Renderer style contract 也锁定了 system `color-scheme`，确保 WebKit 原生控件跟随 Light/Dark。另有 macOS preview UAT 验证 dynamic conversation-window manifest 的 Cmd-Q 保留、下次启动恢复及单窗关闭清理。Runtime 验证必须使用独立的 `HATCH_RUNTIME_DATA_DIR`，避免复用旧的 durable idempotency fixture。
