...
| Factory, Release, Runtime, Desktop, and Dashboard remain Creator/domain agnostic | **Verified** | `docs/proof/v1-connected/release-portability-2026-08-01.json` resolves and materializes Maya's resume product and Ari's unrelated strength-planning product from isolated Release roots with the same Runtime. |
...
| Creator Agent beats a generic baseline on Maya-specific behavior | **Verified** | `connected-run-2026-08-01.json` binds the final digest to the Kimi-only blind comparison: Creator Agent `1.0` versus generic baseline `0.25`, a strict `+0.75` delta. |
...
| Jordan obtains the Agent through one paid entitlement | **Verified** | `desktop-commerce-uat-2026-08-01.json` verifies one entitlement against the final Release and one completed paid delivery. |
| Desktop is a generic Agent surface driven by Release metadata | **Verified locally** | The installed Tauri app proof uses the generic Hatch agent surface against the final Release; renderer tests cover entitlement and product-policy metadata handling. |
...
| Registry and Desktop distribution have production identity/signing | **Not yet proven** | The Registry documents a deterministic development Ed25519 key, and the local macOS UAT does not prove Apple signing/notarization, updates, or production credential rotation. |
