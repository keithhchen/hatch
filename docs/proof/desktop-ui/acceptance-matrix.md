...
| Web build、Tauri app build、DMG、Rust、Runtime、LocalRunner | PASS（本机；Windows 仅 cross-check） | Renderer 103、Rust 44+1 ignored、Runtime 226、LocalRunner 43；`build:web`、`build:app`、`build` 的 strict ad-hoc DMG 校验；`x86_64-pc-windows-gnu cargo check --lib` 使用 rustup target + LLVM-RC/clang 通过 | CI runner 与发布签名链路；Windows installer、真实 Explorer/IME/DPI/Snap/Narrator UAT 仍未证明 |
