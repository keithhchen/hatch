...
| 每窗口 Conversation、Workspace、draft、zoom、scroll、cursor 独立保存 | PARTIAL | native settings schema、account-scoped window context、renderer/native tests；preview 可通过同一 `open_conversation_window` 命令创建多个真实 Tauri conversation windows；Rust 三窗口关闭中间窗口回归测试 | 三个并行真实窗口关闭/重开及 crash/restart E2E；云端 hydration 的真实账户窗口验收 |
