...
| 正式 macOS secure session identity gate | PARTIAL | production flag `cargo check`；Team ID、bundle ID、Developer ID requirement 检查；`desktop-release.yml` 要求 source SHA、Developer ID/notarization、不可变 DMG manifest/hash 和受保护 target UAT 后才接受 signed candidate | 真实 Developer ID + notarization + clean-account restart UAT 与 required-reviewer 记录 |
...
| VoiceOver / Narrator、IME、drag/drop、fullscreen、Snap、DPI | EXTERNAL | ARIA/focus contract、macOS manual tree evidence；Windows 当前暂停 | 真实目标平台人工验收 |
| macOS 自动 UAT 候选包与证据记录 | PARTIAL（CI 配置） | PR、`master` 与手动 CI 构建两个 ad-hoc macOS `.app`→DMG，并上传 package SHA-256、source SHA、runner 与 `HATCH_PERSISTENT_SESSION=0` 的 JSON evidence；另有受保护 self-hosted target-UAT skeleton 复核同一 source/hash 后采集安装、冷启动、截图与日志；详见 [automated CI UAT](automated-ci-uat.md) | 下载后的同一不可变包仍须在真实目标设备安装、启动和完成人工 P4；CI 不能代替 VoiceOver、IME、Finder、DPI 或签名发布验收 |
| Signed/notarized release package identity 与 publication gate | PARTIAL（受保护 workflow contract） | `record-desktop-release-artifact.mjs` 在 notarization/staple 后记录 source SHA、workflow run ID、tag、DMG bytes/SHA-256、signed/persistent-session contract 及 signing identity/Team ID/bundle ID；target job 用 default-branch verifier 复核同一 artifact、run identity 与 codesign provenance；该 lane 只保留 signed candidate 与 UAT evidence，公共 macOS 发布由 `desktop-ci.yml` 的 OSS job 负责；详见 [release UAT contract](release-uat-contract.md) | GitHub Developer ID/notary secrets、`desktop-release-uat` runner approval、真实 interactive macOS runner、clean-account Keychain restart 与完整视觉/Workspace smoke 尚未在本机证明 |
