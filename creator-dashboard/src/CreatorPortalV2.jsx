...
function PreviewPage({ token, request, navigate, productId }) {
  ...
  if (published) {
    ...
    return <section className="cpv2-published" aria-live="polite"><span aria-hidden="true">✓</span><h1>Your Product is live</h1><p>People can now purchase this immutable release at no charge and keep permanent access.</p><label>Share link<input readOnly value={publicUrl ?? "Publication completed"} onFocus={(event) => event.target.select()} /></label><div>{publicUrl ? <Button type="button" onClick={() => copy(publicUrl)}>{copied ? "Copied" : "Copy link"}</Button> : null}{publicUrl ? <Button asChild variant="secondary"><a href={safePublicUrl(publicUrl)} target="_blank" rel="noreferrer">View storefront</a></Button> : null}<Button variant="secondary" type="button" onClick={() => navigate(`${ROOT}/products/${encodeURIComponent(productId)}`)}>Back to Product</Button></div>{error ? <InlineError>{error}</InlineError> : null}</section>;
  }
