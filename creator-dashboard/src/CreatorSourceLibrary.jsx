...
export function CreatorSourceLibrary({ token, taskId, navigate }) {
  ...
  async function createTask(event) {
    ...
    try {
      ...
      navigate(`/studio/tasks/${encodeURIComponent(created.id)}/sources`);
  ...
  async function upload(files) {
    ...
    try {
      ...
      setNotice(`${uploaded.length} source${uploaded.length === 1 ? "" : "s"} uploaded. Original files and projections are retained in this Task's private store.`);
    } catch (nextError) { setError(nextError.message); }
    finally { setBusy(false); }
  }
  ...
  return <section className="cpv2-source-library">
    ...
    <article className="cpv2-card cpv2-panel">
      <div className="cpv2-source-library-toolbar"><div><span className="cpv2-kicker">Private Task storage</span><h2>{selectedCount} source{selectedCount === 1 ? "" : "s"}</h2></div></div>
      <FileUploader multiple accept=".pdf,.docx,.xlsx,.xls,.xlsm,.csv,.tsv,.txt,.md,.json,.html,.htm,.png,.jpg,.jpeg,.webp" onFiles={upload} disabled={busy} label="Upload source files" hint="Local files only; repeat uploads are allowed." className="cpv2-source-uploader" />
      ...
      {documents.length ? <List items={documents} className="cpv2-source-list" ariaLabel="Task source files" renderItem={(document) => <><div><strong>{document.display_name}</strong><small>{document.media_type} · {document.projection?.kind === "image" ? "native image" : "Markdown projection"}</small></div><span>{document.projection?.sha256?.slice(0, 18) ?? "digest pending"}</span></>} /> : <EmptyState title="No source files yet" body="Upload the first source to create this Task's private source area." />}
