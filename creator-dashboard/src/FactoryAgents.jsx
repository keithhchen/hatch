...
export function FactoryAgents({ creatorId, locale = 'en' }) {
  ...
  useEffect(() => { refresh(); refreshConfig(); return subscribeFactoryEvents({ onMessage: e => { const v = JSON.parse(e.data); if (['state', 'todos', 'files'].includes(v.type)) refresh(); }, onError: () => setError('与工作区服务的连接中断，正在重新连接。'), onOpen: () => { setError(''); refresh(); refreshConfig(); } }); }, []);
  const openAgent = async role => {
    try {
      let session = sessions.filter(s => s.role === role).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt))[0];
      if (!session) session = await api('/v1/creator/factory-agents/sessions', { method: 'POST', body: { role } });
  ...
  const uploadProjectFiles = async files => {
    try {
      ...
      if (!transport) transport = await api('/v1/creator/factory-agents/sessions', { method: 'POST', body: { role: 'research' } });
      for (const file of files) {
        if (file.size > 20 * 1024 * 1024) throw new Error(`${file.name} 超过 20 MiB`);
        const bytes = new Uint8Array(await file.arrayBuffer()); let binary = '';
        for (let i = 0; i < bytes.length; i += 8192) binary += String.fromCharCode(...bytes.subarray(i, i + 8192));
        await api(endpoint(transport.id, 'files'), { method: 'POST', body: { path: `input/manual/${file.name}`, base64: btoa(binary), mimeType: file.type || undefined } });
      }
      await refresh();
    } catch (e) { setError(e.message); }
  };
...
function Workspace({ id, sessions, config, onChanged, t }) {
  ...
  useEffect(() => { let live = true; const load = () => api(endpoint(id)).then(v => { if (live) { setSession(v); setError(''); } }).catch(e => { if (live) setError(e.message); }); load(); const unsubscribe = subscribeFactoryEvents({ onMessage: e => { const v = JSON.parse(e.data); if (v.sessionId !== id) return; if (v.type.startsWith('voice.')) voiceHandler.current(v); if (v.type === 'delta') setStream(value => value + v.text); if (v.type === 'message' || v.type === 'state') { setStream(''); load(); } if (v.type === 'tool') setActivity(t('callingTool', v.name)); if (v.type === 'thinking') setActivity(t('thinking')); if (v.type === 'compacting') setActivity(t('compacting')); if (v.type === 'tool_end') setActivity(v.isError ? t('toolErrorContinuing', v.name) : t('continuing')); if (['files', 'todos', 'comments'].includes(v.type)) load(); }, onError: () => { if (live) setActivity(t('reconnecting')); }, onOpen: () => { if (live) { setActivity(''); setStream(''); load(); } } }); return () => { live = false; unsubscribe(); }; }, [id, t]);
  ...
  return <><div className="workspace-switch" role="tablist" aria-label={t('workspacePanels')}><button role="tab" aria-selected={pane === 'chat'} onClick={() => setPane('chat')}>{t('chat')}</button><button role="tab" aria-selected={pane === 'files'} onClick={() => setPane('files')}>{t('files')} <span>{s.files.length}</span></button><span className="pane-state">{t(running ? 'working' : STATUS[s.status])}</span></div><section className="chat-panel" data-active={pane === 'chat'}><div className="workspace-heading"><div><h1>{t(ROLES.find(r => r[0] === s.role)?.[1])}</h1><span className={running ? 'status running' : 'status'}>{t(STATUS[s.status])}</span></div><button className="text-button" onClick={() => perform(async () => setPrompt((await api(endpoint(id, 'prompt'))).content))}>{t('viewPrompt')}</button></div>
    <Binding s={s} config={config} perform={perform} t={t}/><TodoList todos={s.todos || []}/>
