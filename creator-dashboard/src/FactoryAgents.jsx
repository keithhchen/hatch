...
import {
  ...
  Paper, Skeleton, Snackbar, Stack, TextField, Toolbar, Tooltip,
  Typography
} from "@mui/material";
import {
...
export function FactoryAgents({ productId, section, agent, navigate, locale = "en" }) {
  ...
  useEffect(() => { refresh(); refreshConfig(); refreshProduct(); return subscribeFactoryEvents({ url: `${root}/events`, onMessage: event => { const value = JSON.parse(event.data); if (["state", "todos", "files"].includes(value.type)) refresh(); }, onError: () => setError(t("workspaceConnectionLost")), onOpen: () => { setError(""); refresh(); refreshConfig(); refreshProduct(); } }); }, [productId, root, t]);
  ...
  return <Box className="factory-upload-page" sx={{ height: "100%", overflow: "auto", p: { xs: 3, md: 5 }, maxWidth: 980 }}>
    ...
    <Paper variant="outlined" sx={{ overflow: "hidden" }}>
      <Box sx={{ px: 2.5, py: 1.5, display: "flex", alignItems: "center", gap: 1, borderBottom: 1, borderColor: "divider" }}>
        <FolderOpen fontSize="small" /><Typography variant="subtitle2" fontWeight={750}>{t("manualUploads")}</Typography><Chip size="small" label={t("fileCount", manualFiles.length)} sx={{ ml: "auto" }} />
      </Box>
...
function Workspace({ root, id, entry, config, onChanged, onOpenAgent, agentName, t }) {
...
function InspectorPanel({ entry, session, root, id, running, busy, file, setFile, upload, addToChat, agentName, onOpenAgent, useLatest, t, perform }) {
