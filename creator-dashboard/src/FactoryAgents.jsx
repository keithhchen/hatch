...
function NextSteps({ entry, agents, todos, onOpenAgent, onUseLatest, busy, agentName, t }) {
  const gaps = agentDependencyGaps(entry);
  const updatedDependencies = changedDependencies(entry, agents);
  ...
  return <Stack spacing={1} sx={{ mb: 1.5 }}>
    ...
    {updated && <Alert severity="info" action={<Button size="small" disabled={busy} onClick={onUseLatest}>{t("improveWithLatest")}</Button>}><Stack direction="row" spacing={.5} sx={{ flexWrap: "wrap" }}>{updatedDependencies.map(role => <Chip key={role} size="small" color="warning" variant="outlined" label={t("dependencyUpdated", agentName(role))} />)}</Stack></Alert>}
