...
function Workspace({ root, id, entry, agents, sessions, config, onChanged, onOpenAgent, agentName, locale, t }) {
  ...
  useEffect(() => { setSelectedOutputs(paths => paths.filter(path => s?.files.some(record => record.path === path && path.startsWith('output/')))); }, [s?.files]);
  ...
  const destinations = agents.filter(candidate => candidate.role !== s.role && candidate.state !== 'locked').filter(candidate => sessions.find(session => session.role === candidate.role)?.status !== 'running').sort((a, b) => a.order - b.order);
  ...
  const transferOutputs = () => perform(async () => {
    const destination = destinations.find(candidate => candidate.role === destinationRole);
    if (!destination || !selectedOutputs.length) return;
    let target = sessions.filter(session => session.role === destinationRole).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt))[0];
    if (!target) target = await api(`${root}/sessions`, { method: 'POST', body: { role: destinationRole } });
    await api(endpoint(root, target.id, 'transfer'), { method: 'POST', body: { fromSessionId: id, files: selectedOutputs.map(path => ({ path })) } });
    setHandoffNotice(t('filesSent', selectedOutputs.length, agentName(destinationRole)));
    setSelectedOutputs([]);
  });
  return <><div className="workspace-switch" role="tablist" aria-label={t('workspacePanels')}><button role="tab" aria-selected={pane === 'chat'} onClick={() => setPane('chat')}>{t('chat')}</button><button role="tab" aria-selected={pane === 'files'} onClick={() => setPane('files')}>{t('files')} <span>{s.files.length}</span></button><span className="pane-state">{t(running ? 'working' : STATUS[s.status])}</span></div><section className="chat-panel" data-active={pane === 'chat'}><div className="workspace-heading"><div><h1>{agentName(s.role)}</h1><span className={running ? 'status running' : 'status'}>{t(STATUS[s.status])}</span></div><button className="text-button" onClick={() => perform(async () => setPrompt((await api(endpoint(root, id, 'prompt'))).content))}>{t('viewPrompt')}</button></div>
    ...
    <section className="files-panel" data-active={pane === 'files'}><div className="section-heading"><h2>{t('files')}</h2><span className="muted">{t('fileCount', s.files.length)}</span></div><div className="file-trays"><div className="file-tray" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (!running && e.dataTransfer.files.length) upload([...e.dataTransfer.files]); }}><div className="section-heading"><strong>{t('attachments')}</strong><label className={`upload ${running ? 'disabled' : ''}`}>{t('addFiles')}<input aria-label={t('uploadInputFiles')} type="file" multiple disabled={running || busy} onChange={e => { upload([...e.target.files]); e.target.value = ''; }}/></label></div>{s.files.filter(f => f.path.startsWith('input/')).map(f => <div className="file-row" key={f.path}><button className="file-link" onClick={() => { setFile(f.path); setPane('files'); }} title={f.path}>{f.path.slice(6)}</button>{/^input\/(?:manual|handoff)\//.test(f.path) && <button className="remove" aria-label={t('removeFile', f.path)} disabled={running} onClick={() => perform(() => api(endpoint(root, id, 'files'), { method: 'DELETE', body: { path: f.path } }))}>×</button>}</div>)}{!s.files.some(f => f.path.startsWith('input/')) && <p className="empty-tray">{t('dropFiles')}</p>}</div><div className="file-tray"><div className="section-heading"><strong>{t('outputs')}</strong></div>{s.files.filter(f => f.path.startsWith('output/')).map(f => <div className="file-row" key={f.path}><input aria-label={t('selectFile', f.path)} type="checkbox" checked={selectedOutputs.includes(f.path)} onChange={event => { setHandoffNotice(''); setSelectedOutputs(paths => event.target.checked ? [...paths, f.path] : paths.filter(path => path !== f.path)); }}/><button className="file-link" onClick={() => { setFile(f.path); setPane('files'); }} title={f.path}>{f.path.slice(7)}{f.readonly ? ' ◦' : ''}</button></div>)}{!s.files.some(f => f.path.startsWith('output/')) && <p className="empty-tray">{t('noFiles')}</p>}</div></div>
    {s.files.some(record => record.path.startsWith('output/')) ? <div className="handoff"><label><span>{t('sendOutputsTo')}</span><select aria-label={t('destinationAgent')} value={destinationRole} onChange={event => { setDestinationRole(event.target.value); setHandoffNotice(''); }}><option value="">{t('chooseAgent')}</option>{destinations.map(candidate => <option key={candidate.role} value={candidate.role}>{agentName(candidate.role)}</option>)}</select></label><Button variant="secondary" size="compact" disabled={busy || !destinationRole || !selectedOutputs.length} onClick={transferOutputs}>{t('sendFiles', selectedOutputs.length)}</Button>{handoffNotice ? <small role="status">{handoffNotice}</small> : destinations.length ? <small>{t('handoffHint')}</small> : <small>{t('noHandoffDestination')}</small>}</div> : null}
