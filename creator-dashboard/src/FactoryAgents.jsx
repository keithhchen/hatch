...
function SourceUploadWorkspace({ manualFiles, uploadItems, importPicker, uploadProjectFiles, onOpenAgent, t }) {
  return <Box className="factory-upload-page" sx={{ height: "100%", overflow: "auto", p: { xs: 3, md: 5 }, maxWidth: 980 }}>
    ...
    {uploadItems?.length ? <Paper variant="outlined" sx={{ mb: 2, overflow: "hidden" }}><List dense disablePadding>{uploadItems.map(item => <ListItemButton key={item.id} disabled><ListItemIcon><Description fontSize="small" /></ListItemIcon><ListItemText primary={item.file.name} secondary={item.error || t(item.status === "completed" ? "uploadComplete" : item.status === "failed" ? "uploadFailed" : "uploadProcessing")} /><Box sx={{ width: 120, ml: 2 }}>{item.status === "uploading" ? <LinearProgress /> : <Typography variant="caption" color={item.status === "completed" ? "success.main" : "error.main"}>{t(item.status === "completed" ? "uploadComplete" : "uploadFailed")}</Typography>}</Box></ListItemButton>)}</List></Paper> : null}
