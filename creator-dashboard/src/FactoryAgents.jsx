...
export function FactoryAgents({ creatorId, locale = 'en' }) {
  ...
  return <section className="factory-agents" aria-label="Factory">
    <nav className="roles" aria-label="Agent">{ROLES.map(([id, title, description]) => <button key={id} className={role === id ? 'role active' : 'role'} onClick={() => chooseRole(id)} aria-current={role === id ? 'page' : undefined} aria-describedby={`${id}-hint`}><span className="role-full">{t(title)}</span><span className="role-short">{t(id === 'generation' ? 'generationShort' : id === 'case-generation' ? 'caseShort' : title)}</span><span className="role-hint" id={`${id}-hint`} role="tooltip">{t(description)}</span></button>)}</nav>
