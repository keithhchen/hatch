...
export function FactoryAgents({ creatorId }) {
  ...
  return <section className="factory-agents" aria-label="Factory">
    <nav className="roles" aria-label="Agent">{ROLES.map(([id, title]) => <button key={id} className={role === id ? 'role active' : 'role'} onClick={() => chooseRole(id)} aria-current={role === id ? 'page' : undefined}><span className="role-full">{title}</span><span className="role-short">{title === 'Agent Generation' ? 'Generation' : title === 'Case Generation' ? 'Case' : title}</span></button>)}</nav>
...
function Workspace({ id, sessions, config, onChanged }) {
  ...
  useEffect(() => { let live = true; const load = () => api(endpoint(id)).then(v => { if (live) { setSession(v); setError(''); } }).catch(e => { if (live) setError(e.message); }); load(); const unsubscribe = subscribeFactoryEvents({ onMessage: e => { const v = JSON.parse(e.data); if (v.sessionId !== id) return; if (v.type.startsWith('voice.')) voiceHandler.current(v); if (v.type === 'delta') setStream(t => t + v.text); if (v.type === 'message' || v.type === 'state') { setStream(''); load(); } if (v.type === 'tool') setActivity(`正在调用 ${v.name}`); if (v.type === 'thinking') setActivity('正在思考'); if (v.type === 'compacting') setActivity('正在整理上下文'); if (v.type === 'tool_end') setActivity(v.isError ? `${v.name} 返回错误，Agent 正在处理` : '继续工作'); if (['files', 'progress', 'comments'].includes(v.type)) load(); }, onError: () => { if (live) setActivity('连接中断，正在重新连接'); }, onOpen: () => { if (live) { setActivity(''); setStream(''); load(); } } }); return () => { live = false; unsubscribe(); }; }, [id]);
...
function FileViewer({ running, id, record, comments, perform, files, onOpenFile, onAddToChat, resultPath }) {
  ...
  if (!file) return <div className="document-empty">读取文件…<ErrorNotice error={readError}/><ErrorNotice error={error}/></div>;
