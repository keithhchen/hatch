      ...
      <DropdownMenuPrimitive.Portal>
        <DropdownMenuPrimitive.Content className="hui-menu" align={align} sideOffset={7} aria-label={label}>
          {items.map((item, index) => {
            ...
            return <DropdownMenuPrimitive.Item className={cn("hui-menu__item", item.active && "is-active", item.destructive && "is-destructive")} key={item.value || item.label} disabled={item.disabled} onSelect={item.onSelect}>{item.icon}<span>{item.label}</span>{item.shortcut ? <kbd>{item.shortcut}</kbd> : null}</DropdownMenuPrimitive.Item>;
          })}
