...
test("Release delivery workflow hides unsafe drafts and proposed writes until an audited revision passes", async () => {
  ...
  try {
    ...
    for (const request of reviewerRequests) {
      ...
      assert.match(payload.protected_knowledge, /RAG-CAMPAIGN-EVIDENCE/);
...
test("entitlement-bound run materializes protected Skills, retrieved RAG, few-shots, and Release tool policy", async () => {
  const releaseFixture = await createReleaseFixture();
  ...
  assert.match(observed.prompt ?? "", /RAG-CAMPAIGN-EVIDENCE/);
