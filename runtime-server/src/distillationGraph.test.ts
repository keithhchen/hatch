...
test("a new revision clears stale gates and an immutable release is revision-scoped", async () => {
  ...
  await graph.ensureRun({ id: "distill_1", taskId: "task_1", creatorId: "creator_1", createdAt: "2026-08-15T00:00:00.000Z" });
  await graph.createRevision({ id: "factory_1", runId: "distill_1", taskId: "task_1", revision: 1, sourceSnapshotId: "snapshot_1", createdAt: "2026-08-15T00:00:00.000Z" });
  await graph.createRevision({ id: "factory_2", runId: "distill_1", taskId: "task_1", revision: 2, sourceSnapshotId: "snapshot_2", parentRevisionId: "factory_1", createdAt: "2026-08-15T00:01:00.000Z" });
  ...
  await graph.recordGate({ id: "g1", gateKey: "factory_1:regression", taskId: "task_1", runId: "distill_1", revisionId: "factory_1", name: "regression", critical: true, status: "failed", evidenceArtifactIds: [], assessedAt: "2026-08-15T00:00:10.000Z" });
  ...
  await graph.recordGate({ id: "g2", gateKey: "factory_2:heldout", taskId: "task_1", runId: "distill_1", revisionId: "factory_2", name: "heldout", critical: true, status: "passed", evidenceArtifactIds: ["art_corpus_2"], assessedAt: "2026-08-15T00:01:30.000Z" });
  // A late worker retry from v1 must not move the Task's current revision
  ...
  // Source Library uploads are Task-scoped and may arrive after a revision;
  ...
  const release = await graph.recordRelease({ id: "release_1", taskId: "task_1", runId: "distill_1", revisionId: "factory_2", productId: "product_1", corpusArtifactId: "art_corpus_2", createdAt: "2026-08-15T00:02:00.000Z" });
...
test("revision context is n-1 plus current feedback plus cumulative regression", () => {
  const revisions = [
    ...
    { id: "r2", runId: "distill_1", taskId: "task_1", revision: 2, sourceSnapshotId: "snapshot_2", parentRevisionId: "r1", createdAt: "2026-08-15T00:01:00.000Z" }
  ];
  const events = [
...
test("graph records cannot cross task, run, or revision boundaries", async () => {
  const graph = new InMemoryDistillationGraphStore();
  await graph.ensureRun({ id: "distill_1", taskId: "task_1", creatorId: "creator_1", createdAt: "2026-08-15T00:00:00.000Z" });
  await assert.rejects(
    () => graph.ensureRun({ id: "distill_2", taskId: "task_1", creatorId: "creator_1", createdAt: "2026-08-15T00:00:00.000Z" }),
    /already attached/
  );
  await graph.createRevision({ id: "factory_1", runId: "distill_1", taskId: "task_1", revision: 1, sourceSnapshotId: "snapshot_1", createdAt: "2026-08-15T00:00:00.000Z" });
  await assert.rejects(
    () => graph.recordGate({ id: "bad_gate", gateKey: "bad", taskId: "task_1", runId: "other_run", revisionId: "factory_1", name: "schema", critical: true, status: "pending", evidenceArtifactIds: [] }),
    /invalid RunRevision/
  );
});
