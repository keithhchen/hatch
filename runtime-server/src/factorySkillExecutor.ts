...
async function main(): Promise<void> {
  ...
  for (let attempt = 0; attempt <= MAX_REPAIR_ATTEMPTS; attempt += 1) {
    try {
      ...
      const routing = envelope.factory_plan?.runtime_knowledge_source_ids;
      if (!Array.isArray(routing) || routing.some((item: unknown) => typeof item !== "string")) {
        throw new Error("Factory Agent must explicitly route runtime_knowledge_source_ids; use [] when no retrieval-only knowledge is needed");
      }
      ...
      const agentCorpusPath = path.join(String(compiledRelease.path), "agent");
      const agentCorpus = await readJson<{
        agent_id?: unknown;
        creator?: { id?: unknown };
      }>(path.join(agentCorpusPath, "agent.json"));
      if (typeof agentCorpus.agent_id !== "string" || typeof agentCorpus.creator?.id !== "string") {
        throw new Error("Compiler produced an Agent Corpus without creator.id and agent_id");
      }
      const registryPublication = args.publish
        ? await publishAgentCorpus({
            registryUrl: process.env.HATCH_REGISTRY_URL,
            serviceToken: process.env.HATCH_REGISTRY_PUBLISH_SERVICE_TOKEN,
            corpusPath: agentCorpusPath,
            creatorId: agentCorpus.creator.id,
            agentId: agentCorpus.agent_id
          })
        : undefined;
      await writeJson(path.join(output, "agent-run.json"), {
        ...
        agent_corpus: {
          path: agentCorpusPath,
          creator_id: agentCorpus.creator.id,
          agent_id: agentCorpus.agent_id,
          published: Boolean(registryPublication)
        },
        ...(registryPublication ? { registry_publication: registryPublication } : {}),
...
function parseArgs(argv: string[]): Arguments {
  ...
  if (preflight) return { preflight: true, publish: false };
  ...
  if (resume) return { resume, preflight: false, publish };
  if (!input || !intent || !output) {
    throw new Error("Usage: factorySkillExecutor --input <creator-material-directory> --intent-file <intent.txt> --output <private-output> [--publish] [--preflight]");
  }
  return { input, intent, output, preflight: false, publish };
}
...
function factorySystemPrompt(input: { skill: string; workflow: string; contract: string }): string {
  return [
    ...
    "Every Creator-authority claim must be an exact source substring. Derived rules need two or more independent support IDs. Synthetic QA and held-outs must be clearly synthetic. Select only high-signal direct or composed QA ids for `few_shot_ids`; never select boundary, out-of-scope, or held-out tests as live demonstrations. Explicitly choose `runtime_knowledge_source_ids` for long-tail retrieval only; do not put behavior rules, global few-shots, or local Skill material there. Use `skill_units` only for independently reusable execution units; use [] when no Skill is needed, and put local methods or scoped few-shots in that unit's references.",
...
function outputShape(): string {
  return [
    ...
    "Use the compiler's exact field names. In particular source_manifest.product must use `version` (not `semantic_version`) and source_manifest.documents must carry `origin_source_id` and `origin_path` from intake. Do not emit a root `tenant_id`: the clean Corpus identity is `creator.id + agent_id`; source_manifest is private provenance only. source_manifest must comply with input-contract.md and use only intake source IDs/provenance. factory_plan must fully comply with input-contract.md. `claim_annotations` is an object keyed by claim ID. `factory_plan.method.quality_bar`, `.omissions`, `.boundaries`, and `.priorities` are flat non-empty arrays of source-fact or derived-rule IDs—never descriptive objects. Every `factory_plan.method.phases` row must use `supports` (a non-empty flat array of existing source-fact or derived-rule IDs; never `supporting_claims`). `runtime_knowledge_source_ids` is required; use [] when no retrieval-only long-tail material is needed. It must be a flat list of declared source IDs and contain only retrieval-only long-tail documents. `skill_units` is optional; when present it must be [] or a list of independently reusable units with id, name, when_to_use, instruction, and optional references. `few_shot_ids` is optional and may contain only ids from direct or composed QA rows. Every `held_out_evals` row must include `id`, `category` (one of direct|composed|boundary|out_of_scope), `input`, `expected_behavior`, `observable_checks`, `forbidden_behavior`, `supports`, and `generic_baseline_risk`. Preserve all internal fields even when empty arrays would be invalid; fail closed instead of guessing."
  ].join("\n");
}
...
async function publishAgentCorpus(input: {
  registryUrl?: string;
  serviceToken?: string;
  corpusPath: string;
  creatorId: string;
  ...
  const registryUrl = input.registryUrl?.trim();
  const serviceToken = input.serviceToken?.trim();
  if (!registryUrl || !serviceToken) {
    throw new Error("--publish requires HATCH_REGISTRY_URL and HATCH_REGISTRY_PUBLISH_SERVICE_TOKEN");
  }
  const bundle = await zipCorpusDirectory(input.corpusPath);
  const url = new URL("/v1/agent-corpora", registryUrl);
    url.searchParams.set("creator_id", input.creatorId);
    ...
    if (!response.ok) {
      ...
      throw new Error(`Registry Agent Corpus publication failed: ${detail}`);
    }
  return payload;
}

async function zipCorpusDirectory(root: string): Promise<Uint8Array> {
  ...
  async function visit(directory: string): Promise<void> {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      else throw new Error(`Unsupported Agent Corpus entry: ${relative}`);
    }
  }
  await visit(root);
  if (!files["agent.json"]) throw new Error("Agent Corpus must contain agent.json before Registry publishing");
