...
async function complete(client: OpenAI, model: string, system: string, input: string): Promise<string> {
  const request: any = { model, temperature: KIMI_TEMPERATURE, thinking: KIMI_THINKING, max_completion_tokens: 1800, messages: [{ role: "system", content: system }, { role: "user", content: input }] };
...
async function judge(client: OpenAI, model: string, item: HeldOut, answerA: string, answerB: string): Promise<{ a_passed: boolean; b_passed: boolean; a_score: number; b_score: number; rationale: string }> {
  const request: any = {
    model, temperature: KIMI_TEMPERATURE, thinking: KIMI_THINKING, response_format: { type: "json_object" }, max_completion_tokens: 1200,
