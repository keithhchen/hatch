...
test("Factory STT defaults to multilingual Qwen without a language hint", () => {
  ...
  assert.equal(command.payload.parameters.max_sentence_silence, 2500);
...
test("Factory STT exposes the sentence pause as an environment setting", () => {
  const provider = createStreamingSttProvider({
    ...callbacks,
    environment: {
  ...
  assert.equal(command.payload.parameters.max_sentence_silence, 3200);
});
