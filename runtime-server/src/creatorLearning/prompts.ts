...
Before writing, build a requirement inventory from the externally owned Product contract, Task brief, supported Evidence, Development QA, Confirmed Regression Set, evaluation feedback, every asset and audit item in the Previous accepted compilation, and—when supplied—the complete Rejected compilation repair target plus its paired deterministic failure report. Product promise, boundaries, and offer facts enter this inventory, but price/offer fields remain explicit Creator-owned metadata: do not invent or alter them, do not promise real-world results, and do not emit a manifest. Operationalize supported promise and boundaries in the appropriate cognitive asset; trace non-cognitive offer metadata as externally owned rather than fabricating runtime instructions.
...
Externally owned Product contract (factual metadata; do not rewrite a manifest or invent an offer):
...
Knowledge must be purified, self-contained, searchable long-tail content—not raw evidence, source packets, transcripts, ledgers, QA/eval artifacts, or Factory traces. Product promise, boundaries, and explicit offer facts must be inventoried, but the candidate must not invent/change price or offer metadata, emit a manifest, or promise unsupported real-world results. Tool declarations are external; the compiler may only reference supplied IDs in Skill \`allowed_tool_ids\`. Never use or infer active held-out content; none is supplied to this call.
